HA$PBExportHeader$u_pbunit_progressbar.sru
$PBExportComments$Progress bar for test runner window.  Copied from the PFC ProgressBar class
forward
global type u_pbunit_progressbar from userobject
end type
type dw_progress from datawindow within u_pbunit_progressbar
end type
end forward
...
global u_pbunit_progressbar u_pbunit_progressbar
...
event ue_resize;// expand dw to dimensions of on screen object
dw_progress.height = newheight
dw_progress.width = newwidth

// make the dw header band height of on screen object
// This value seems can't be repeatly set.
//dw_progress.object.datawindow.header.height = newheight

// make the progress rectangle the height and width of on screen object 
dw_progress.object.progress_rect.height = newheight
dw_progress.object.progress_rect.width = newwidth

// center percentage text on screen object
dw_progress.object.pct.y = &
	integer((integer(dw_progress.object.datawindow.header.height) / 2 ) - &
...
this.event ue_resize(0, this.width, this.height)
...
integer width = 1147
integer height = 76
string dataobject = "d_pbunit_progress"
